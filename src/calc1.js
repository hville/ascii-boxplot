...
function step(itm) {
	...
	while (itm.r > 0) {
		...
		var oldr = itm.r
		//console.log(itm.r)
		//itm.r -= Math.max(0, Δr(itm.N, itm.F, itm.r, itm.b))
	...
	//console.log(itm.pts[3])
	//console.log(itm.xys[3])
	//console.log(itm.pts[itm.pts.length-4])
	itm.xys = itm.pts
...
module.exports = function(da) {
	...
	for (var i=0; i<N; ++i) {
		...
		itms[i] = step(curve(
			[0,0], //center
			0.22 + 0.14*Math.cos(angle), //radius
			0.02 - 0.015*Math.cos(angle), //width
			angle-da, //direction
			4.5 - 4*Math.cos(angle), //turns
			2.9 + 0.5*Math.cos(angle), //escape
			20 // frequency
		))
		itms[i].xys.forEach(function(xy) {
			//console.log('r', Math.sqrt(xy[0]*xy[0]+xy[1]*xy[1]))
		})
	}
	return itms
}
